...
//    uint64           _remainderBitCount[_numBuckets] = { 0 };
    ...
    size_t           _bucketSize                     = 0;             // Total bytes written so far for this source bucket (before a submit() call)
    byte*            _writeBuffer                    = nullptr;
public:
...
//        SetBlockBuffers( blockBuffers );
    ...
    inline void BeginWriteBuckets( const uint64 bucketBitSizes[_numBuckets] )
    {
        ...
        for( uint32 i = 0; i < _numBuckets; i++ )
            allocSize += CDivT( (size_t)bucketBitSizes[i], fsBlockSizeBits ) * fsBlockSizeBits / 8;
//        for( uint32 i = 0; i < _numBuckets; i++ )
...
//            }
            ...
            _bucketSize += bufferBitCapacity / 8;
        }
    }

    //-----------------------------------------------------------
    inline void Submit( const uint32 writeBucket = 0 )
    {
        // Round-up to block size
        const size_t fsBlockSize = _queue->BlockSize( _fileId );
        ASSERT( _bucketSize == RoundUpToNextBoundaryT( _bucketSize, fsBlockSize ) );

        // Round-up to block size
        if( _bucketSize )
        {
            _queue->WriteFile( _fileId, writeBucket, _writeBuffer, _bucketSize );
...
//        _queue->CommitCommands();
    ...
    //-----------------------------------------------------------
    inline void SubmitAndRelease( const uint32 writeBucket )
    {
        this->Submit( writeBucket );
...
//        _queue->CommitCommands();
    ...
    inline BitWriter GetWriter( const uint32 bucket, const uint64 bitOffset )
    {
        ...
        return BitWriter( (uint64*)b.buffer, b.count, bitOffset );
    }
